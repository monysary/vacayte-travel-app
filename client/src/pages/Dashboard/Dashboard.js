...
function Dashboard() {
  ...
  const [loadTrip, { loading: lazyLoading, data: lazyData }] = useLazyQuery(
    SELECT_TRIP,
    {
  ...
  const lazyStartDate = new Date(lazyTrip.startDate).toLocaleDateString(
    "en-us",
    {
      month: "2-digit",
      day: "2-digit",
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <Grid container component="main" wrap="nowrap" sx={{ height: "100vh" }}>
        <Grid className="hide-mobile" item md={2} sm={3} xs={0}>
          <LeftPanel
            font={font.primary}
            fontColor={font.color}
            isDisplayed={isDisplayed}
